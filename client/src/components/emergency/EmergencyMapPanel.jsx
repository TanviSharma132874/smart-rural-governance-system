...
function EmergencyMapPanel({ points = [], title = "Emergency Map", subtitle = "Live incident and infrastructure locations" }) {
  ...
  return (
    <section className="glass-panel rounded-[32px] border border-white/70 bg-white/92 p-5">
      <p className="text-xs font-semibold uppercase tracking-[0.28em] text-leaf-600">Map Layer</p>
      <h2 className="mt-2 font-display text-2xl text-ink-950">{title}</h2>
      <p className="mt-2 text-sm leading-6 text-ink-800">{subtitle}</p>

      <div className="mt-5 overflow-hidden rounded-[24px] border border-slate-200">
        <MapContainer center={center} zoom={10} scrollWheelZoom={false} className="h-[320px] w-full">
          <TileLayer
          ...
          {validPoints.map((point) => (
            <CircleMarker
              ...
              radius={10}
              pathOptions={{ color: point.color || "#0f766e", fillColor: point.color || "#16a34a", fillOpacity: 0.75 }}
            >
              <Popup>
                <div className="space-y-1">
                  <p className="font-semibold text-ink-950">{point.title}</p>
                  {point.subtitle ? <p className="text-sm text-ink-800">{point.subtitle}</p> : null}
                </div>
              </Popup>
            </CircleMarker>
          ))}
