...
const enforceJurisdictionPayload = (req, _res, next) => {
  ...
  if (["departmentOfficer", "panchayatOfficer", "districtAdmin", "stateAdmin"].includes(req.user.role)) {
    if (req.user.state && req.user.state !== state) {
      return next(new AppError("State jurisdiction mismatch", 403));
    }

    if (req.user.role !== "stateAdmin" && req.user.district && req.user.district !== district) {
